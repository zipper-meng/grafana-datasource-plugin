...
import { regexEscape } from './grafana_kbn';
...
export default class InfluxQueryModel {
  ...
  /** @ngInject */
  constructor(target: InfluxQuery, scopedVars?: ScopedVars) {
    this.target = target;
  ...
  getMeasurementAndPolicy(interpolate: any) {
    ...
    if (!measurement.match('^/.*/$')) {
      console.log("Get measurement & policy using raw");
      measurement = '"' + measurement + '"';
    } else if (interpolate) {
      console.log("Get measurement & policy using interpolate");
