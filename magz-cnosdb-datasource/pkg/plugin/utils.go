...
func BuildQuery(query backend.DataQuery) (*QueryModel, error) {
	model := QueryModel{}
	jsonDec := json.NewDecoder(bytes.NewBuffer(query.JSON))
	if err := jsonDec.Decode(&model); err != nil {
		return nil, err
	}

	useRawQuery := model.Get("rawQuery").MustBool(false)
	rawQuery := model.Get("query").MustString("")
	if useRawQuery && rawQuery != '' {

	}

	alias := model.Get("alias").MustString("")
	limit := model.Get("limit").MustString("")
	orderBy := model.Get("orderBy").MustString("")
	table := model.Get("table").MustString("")
}
...
func BuildResponse(buf io.ReadCloser, queries: rawQueries) *backend.QueryDataResponse (
