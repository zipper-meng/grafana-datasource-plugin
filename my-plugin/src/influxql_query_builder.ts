...
export class InfluxQueryBuilder {
  constructor(private target: { table: any; tags: any; }, private database?: string) {}
  ...
  buildExploreQuery(type: string, withKey?: string, withMeasurementFilter?: string): string {
    ...
    if (type === 'TAG_KEYS') {
      ...
      if (withMeasurementFilter) {
        ...
        query += ' WITH MEASUREMENT =~ /(?i)' + regexEscape(withMeasurementFilter) + '/';
      }
    } else if (type === 'FIELDS') {
      measurement = this.target.table;
